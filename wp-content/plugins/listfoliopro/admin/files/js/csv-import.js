...
function listfoliopro_update_csv_file(){
		"use strict";
	   var ajaxurl = dirpro_data.ajaxurl;		  	  
	   var search_params={
		 "action"  :  "listfoliopro_finalerp_csv_product_upload",
...
function listfoliopro_save_csv_file_to_database(){
	"use strict";
	listfoliopro_upload_status_checker();
	...
	jQuery("#ep3").show();
	var search_params2={
		"action"  :  	"listfoliopro_save_csv_file_to_database",
	...
	jQuery.ajax({
		 ...
		 success : function(response){	
			...
			if(response.code=='not-done'){ 
				...
				listfoliopro_save_csv_file_to_database();
...
function listfoliopro_upload_status_checker(){
	"use strict";
	var importTimer;		
	...
	importTimer = setInterval(function() {
		// Get the current status of the update
			...
			var search_params3={
					"action"  :  "listfoliopro_eppro_get_import_status",
			};
			jQuery.ajax({
				 url : ajaxurl,
				 dataType : "json",
...
function listfoliopro_upload_csv_file(update_div_id){
	"use strict";
