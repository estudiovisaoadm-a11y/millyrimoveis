...
function listfoliopro_epluginrecaptchaSubmit(token){
	"use strict";		
		
	var formc = jQuery("#listfoliopro_registration");
	if (jQuery.trim(jQuery("#iv_member_user_name",formc).val()) == "" || jQuery.trim(jQuery("#iv_member_email",formc).val()) == "" || jQuery.trim(jQuery("#iv_member_password",formc).val()) == "") {	
		...
		if( dirpro_data.iv_gateway=='stripe'){
			...
			}, listfoliopro_stripeResponseHandler);
						
		}else{ /* Else for paypal */
				
				jQuery('#listfoliopro_registration').on( "submit");		
...
(function($) {	
	...
	jQuery(document).ready(function($) {
						...
						jQuery.validate({
							form : '#listfoliopro_registration',
							modules : 'security',		
	 ...
	 // this identifies your website in the createToken call below	
			...
			function listfoliopro_stripeResponseHandler(status, response) {
				if (response.error) {				
					...
					var form$ = jQuery("#listfoliopro_registration");
...
jQuery(document).ready(function() {
    jQuery('#coupon_name').on('keyup change', function() {				
		...
		var search_params={
			"action"  				: "listfoliopro_check_coupon",	
...
function listfoliopro_show_coupon(){
	"use strict";
	jQuery("#coupon-div").show();
	 jQuery("#show_hide_div").html('<label for="text" class="col-md-4 col-xs-4 col-sm-4 control-label"></label><div class="col-md-8 col-xs-8 col-sm-8 " ><button type="button" onclick="listfoliopro_hide_coupon();"  class="btn btn-small-ar center">'+dirpro_data.HideCoupon+'</button></div>');
}
function listfoliopro_hide_coupon(){
	"use strict";
	 jQuery("#coupon-div").hide();
	 jQuery("#show_hide_div").html('<label for="text" class="col-md-4 col-xs-4 col-sm-4 control-label"></label><div class="col-md-8 col-xs-8 col-sm-8 " ><button type="button" onclick="listfoliopro_show_coupon();"  class="btn btn-small-ar center">'+dirpro_data.Havecoupon+'</button></div>');
}