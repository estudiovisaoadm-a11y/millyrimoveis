...
jQuery( function() {	
	...
	jQuery('.all_checked').on("click", function(){	
		const isChecked = jQuery(".all_checked").is(":checked");
		var idcheck= jQuery(this).attr('id');	
		
		if(isChecked == true){	
			jQuery("."+idcheck).prop("checked",true);
		}else{	
			jQuery("."+idcheck).prop("checked", false);
		}
...
function listfoliopro_field_image_fun(field_id){
	"use strict";
	var image_gallery_frame;
	image_gallery_frame = wp.media.frames.downloadable_file = wp.media({
		// Set the title of the modal.
	...
	image_gallery_frame.on( 'select', function() {
		...
		selection.map( function( attachment ) {
			attachment = attachment.toJSON();
			if ( attachment.id ) {
				jQuery('#meta_image_display'+field_id).html('<img width="80px" src="'+attachment.url+'">'); 
				jQuery('#meta_image_id'+field_id).val(attachment.id);
				jQuery('#meta_image'+field_id).val(attachment.url);
			}
		});
	});
	image_gallery_frame.open(); 
