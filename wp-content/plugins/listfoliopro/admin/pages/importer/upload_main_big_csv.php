<?php
	...
	update_option( 'listfoliopro_total_row',$total_rows-1);
	update_option( 'listfoliopro_current_row','1');
	...
	<thead>
    <tr>    
	<th>'.esc_html__('Post Field/Map to field', 'listfoliopro' ).'</th>
	<th>'.esc_html__('CSV Column Title/Name', 'listfoliopro' ).'</th>      
    </tr>
	</thead>';
	foreach($title_row_array as $one_col){
		$sel_name= str_replace (' ','-', $one_col);
		$maping=$maping.'<tr><td><select name="'.trim($sel_name).'">';
		$maping=$maping.'<option value="">'.esc_html__('Email', 'listfoliopro' ).'</option>';
		$ii=0;
