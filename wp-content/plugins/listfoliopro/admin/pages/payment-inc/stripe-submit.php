<?php
	if( isset($_POST['iv-submit-stripe']) && $_POST['payment_gateway']=='stripe' && $_POST['iv-submit-stripe']=='register' ){	
		...
		$stripe_currency =get_post_meta($newpost_id, 'listfoliopro_stripe_api_currency',true);	
		...
		if(trim(get_post_meta( $package_id,'listfoliopro_stripe_plan',true))!=''){
			$package_post_name= trim(get_post_meta( $package_id,'listfoliopro_stripe_plan',true));
		}
		...
		if($package_cost >0){
			...
			if($listfoliopro_package_recurring!='on'){
				...
				if(isset($post_count->post_title)){
					...
					if($today_time >= $start_time && $today_time<=$expire_time && $coupon_used<=$coupon_limit && $pac_found == '1' ){	
						...
						update_post_meta($post_count->ID, 'listfoliopro_coupon_used', $coupon_used+1);
			...
			}	// for if($listfoliopro_package_recurring!='on'){
			// Coupon End *****************************
			...
			if($listfoliopro_package_recurring=='on'){
				...
				if( $trial_enable=='yes' ){
					...
					$package_cost_int=get_post_meta($package_id, 'listfoliopro_package_recurring_cost_initial', true);								
				...
				update_user_meta($userId, 'listfoliopro_payment_gateway','stripe'); 								
				if(isset($customer->id)){
					update_user_meta($userId, 'listfoliopro_stripe_cust_id', $customer->id);
				}							
				if(isset($customer->subscriptions->data[0]->id)){
					update_user_meta($userId, 'listfoliopro_stripe_subscrip_id', $customer->subscriptions->data[0]->id);
				}
				...
				if($payment_status=='active' or $payment_status=='trialing' ){
					...
					$exprie = gmdate('Y-m-d',$timeToBegin).'T'.'00:00:00Z';
					update_user_meta($userId, 'listfoliopro_exprie_date',$exprie); 
				...
				$response='';	
				try {
					$stripe_cost= (int)$package_cost - (int)$dis_amount;
