	...
	if( isset($_REQUEST['iv-submit-listing']) && isset($_REQUEST['payment_gateway']) && $_REQUEST['iv-submit-listing']=='register' && $_REQUEST['payment_gateway']=='paypal'){	
		...
		if($userdata['user_login']!='' and $userdata['user_email']!='' and $userdata['user_pass']!='' ){
			...
			if ( !$user_id and email_exists($userdata['user_email']) == false ) {							
				...
				$expire_interval = get_post_meta($package_id, 'listfoliopro_package_initial_expire_interval', true);						
				$initial_expire_type = get_post_meta($package_id, 'listfoliopro_package_initial_expire_type', true);
				if($expire_interval!='' AND $initial_expire_type!=''){
					...
					$exp_d = gmdate('Y-m-d',$exp_time).'T'.'00:00:00Z';
					}else{
					$exp_d=gmdate('Y-m-d', strtotime('+10 year'));
				} 							  
				update_user_meta($user_id, 'listfoliopro_exprie_date',$exp_d); 
				update_user_meta($user_id, 'listfoliopro_package_id',$package_id);				
				...
				require_once( listfoliopro_ep_ABSPATH. 'inc/signup-mail.php');
				} else {
				$iv_redirect = get_option('listfoliopro_registration');
				if(trim($iv_redirect)!=''){
		...
		$listfoliopro_package_cost= get_post_meta( $package_id,'listfoliopro_package_cost',true);
		$listfoliopro_package_recurring= get_post_meta( $package_id,'listfoliopro_package_recurring',true);
		$package_cost=$listfoliopro_package_cost;
		// Cheek here Coupon ************
		$trial_enable= get_post_meta( $package_id,'listfoliopro_package_enable_trial_period',true);
		if( $listfoliopro_package_recurring=='on'  ){
			$package_cost=get_post_meta($package_id, 'listfoliopro_package_recurring_cost_initial', true);			
		}
		if($package_cost >0){
			$currencyCode = (isset($paypal_api_currency)) ? $paypal_api_currency : 'USD';
			$sandbox = ($paypal_mode == 'live') ? '' : 'sandbox.';
			$paymentName = $package_name;
			$paymentDescription = $listfoliopro_package_cost.' '.$currencyCode.' for '.$package_name .' at '. get_bloginfo();
			$page_name_registration=get_option('listfoliopro_registration' ); 
			...
			if($listfoliopro_package_recurring=='on'){
				$period= get_post_meta( $package_id,'listfoliopro_package_recurring_cycle_type',true);
				...
				$dis_amount=0;
				$package_amount=$listfoliopro_package_cost;
				$post_count = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->posts WHERE post_title = '%s' and  post_type='listfoliopro_coupon'",$coupon_code ));	
				if(isset($post_count->post_title)){
					$coupon_name = $post_count->post_title;							 
					$current_date=$today = gmdate("m/d/Y");							 
					$start_date=get_post_meta($post_count->ID, 'listfoliopro_coupon_start_date', true);
					$end_date=get_post_meta($post_count->ID, 'listfoliopro_coupon_end_date', true);
					$coupon_used=get_post_meta($post_count->ID, 'listfoliopro_coupon_used', true);
					$coupon_limit=get_post_meta($post_count->ID, 'listfoliopro_coupon_limit', true);
					$dis_amount=get_post_meta($post_count->ID, 'listfoliopro_coupon_amount', true);							 
					$package_ids =get_post_meta($post_count->ID, 'listfoliopro_coupon_pac_id', true);
					...
					if($today_time >= $start_time && $today_time<=$expire_time && $coupon_used<=$coupon_limit && $pac_found == '1' ){	
						$coupon_type= get_post_meta($post_count->ID, 'listfoliopro_coupon_type', true);
						update_post_meta($post_count->ID, 'listfoliopro_coupon_used', (int)$coupon_used+1);
				...
				// Coupon End *****************************
				$listfoliopro_package_cost = (int)$listfoliopro_package_cost- (int)$dis_amount;
				$orderParams = array(
				'PAYMENTREQUEST_0_AMT' => $listfoliopro_package_cost,
				'PAYMENTREQUEST_0_SHIPPINGAMT' => '0',
				'PAYMENTREQUEST_0_CURRENCYCODE' => $currencyCode,
				'PAYMENTREQUEST_0_ITEMAMT' => $listfoliopro_package_cost
				);
				$itemParams = array(
				'L_PAYMENTREQUEST_0_NAME0' => $paymentName,
				'L_PAYMENTREQUEST_0_DESC0' => $paymentDescription,
				'L_PAYMENTREQUEST_0_AMT0' =>  $listfoliopro_package_cost,
			...
			if(is_array($response) && $response['ACK'] == 'Success') {
				...
				$peried=get_post_meta( $package_id,'listfoliopro_package_recurring_cycle_type',true);
				$cycle_count=get_post_meta( $package_id,'listfoliopro_package_recurring_cycle_count',true);
				update_user_meta($userId, 'iv_paypal_recurring_profile_amt', get_post_meta( $package_id,'listfoliopro_package_recurring_cost_initial',true));
				update_user_meta($userId, 'listfoliopro_payment_gateway','paypal-express'); 
				update_user_meta($userId, 'iv_paypal_recurring_profile_init_amt',get_post_meta( $package_id,'listfoliopro_package_cost',true));
				...
				update_user_meta($userId, 'listfoliopro_discount',$dis_amount);
				update_user_meta($userId, 'listfoliopro_package_id',$package_id);							
			...
			if(isset($userId)){ 
				...
				$role_package= get_post_meta( $package_id,'listfoliopro_package_user_role',true); 
				if($role_package==""){
					$role_package='basic';
				}
		...
		if($tokenRow){	
			...
			if( is_array($checkoutDetails) && ($checkoutDetails['ACK'] == 'Success') ) {
				...
				if($listfoliopro_package_recurring=='on'){
					...
					$recurring_cycle_count=	get_post_meta($package_id,'listfoliopro_package_recurring_cycle_count',true);
					...
					if( $trial_enable=='yes'  ){
						$initAmt='0';
						$period = get_post_meta( $package_id,'listfoliopro_package_recurring_trial_type',true);						
						$recurring_cycle_count=	get_post_meta( $package_id,'listfoliopro_package_trial_period_interval',true);	
					}	
					...
					$begins = gmdate('Y-m-d',$timeToBegin).'T'.'00:00:00Z';
					$discount='0';
					...
					$arb_limit	= get_post_meta( $package_id,'listfoliopro_package_recurring_cycle_limit',true); 
					if($arb_limit!=''){
						$recurringParams['TOTALBILLINGCYCLES']=$arb_limit;
					}
					$trial_enable= get_post_meta( $package_id,'listfoliopro_package_enable_trial_period',true); 
					if( $trial_enable=='yes' ){					
						$trial_amount= get_post_meta( $package_id,'listfoliopro_package_trial_amount',true); 
						$trial_period_interval= get_post_meta( $package_id,'listfoliopro_package_trial_period_interval',true); 
						$trial_type = get_post_meta( $package_id,'listfoliopro_package_recurring_trial_type',true);
					...
					if( is_array($recurringPayment) && $recurringPayment['ACK'] == 'Success' ) {
						...
						update_user_meta($userId, 'listfoliopro_exprie_date',$begins); 
						update_user_meta($userId, 'listfoliopro_package_id',$package_id);
						// Check here for success status*****	
						if($recurringPayment['PROFILESTATUS']=='PendingProfile'){
							update_user_meta($userId, 'listfoliopro_payment_status', 'pending');
							}else{
							$role_package= get_post_meta( $package_id,'listfoliopro_package_user_role',true); 
							update_user_meta($userId, 'listfoliopro_payment_status', 'success');
							$user = new WP_User( $userId );
							$user->set_role($role_package);						
						}
						//Start aktar mail module  
						$dis_amount=$discount;	
						require_once( listfoliopro_ep_ABSPATH.'inc/order-mail.php');
					...
					if( is_array($singlePayment) && $singlePayment['ACK'] == 'Success') {
						...
						update_user_meta($userId, 'listfoliopro_exprie_date',$exp_d); 
						update_user_meta($userId, 'listfoliopro_package_id',$package_id);
						// End expire date
	...
	if( isset($_REQUEST['iv-submit-upgrade']) &&  $_REQUEST['iv-submit-upgrade']=='upgrade' && $_REQUEST['payment_gateway']=='paypal' ){	
		...
		if($package_cost >0){
			...
			if(is_array($response) && $response['ACK'] == 'Success') {
				...
				update_user_meta($userId, 'iv_paypal_recurring_profile_period',$peried);	
				update_user_meta($userId, 'iv_paypal_recurring_cycle_count',$cycle_count);
				update_user_meta($userId, 'listfoliopro_discount','');
				update_user_meta($userId, 'iv_paypal_recurring_profile_desc',$recurringDescriptionFull);
			}	
			wp_redirect( 'https://www.'.$sandbox.'paypal.com/webscr?cmd=_express-checkout&token=' . urlencode($token) ); 
			exit; 
