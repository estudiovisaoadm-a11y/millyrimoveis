...
function listfoliopro_taxonomy_add_custom_field() {
...
add_action( $listfoliopro_directory_url.'-locations_add_form_fields', 'listfoliopro_taxonomy_add_custom_field', 10, 2 );
 
function listfoliopro_taxonomy_edit_custom_field($term) {
    $image = get_term_meta($term->term_id, 'listfoliopro_term_image', true);
	$nonce = wp_create_nonce('listfoliopro');
    ?>
    <tr class="form-field term-image-wrap">
        <th scope="row"><label for="category_image_url"><?php esc_html_e('Image [Best: 250px X 380px]','listfoliopro');?></label></th>
        <td>
...
add_action( $listfoliopro_directory_url.'-locations_edit_form_fields', 'listfoliopro_taxonomy_edit_custom_field', 10, 2 );

// Save data
add_action('created_'.$listfoliopro_directory_url.'-locations', 'listfoliopro_save_term_image', 10, 2);
function listfoliopro_save_term_image($term_id, $tt_id) {
...
add_action('edited_'.$listfoliopro_directory_url.'-locations', 'listfoliopro_update_image_upload', 10, 2);
function listfoliopro_update_image_upload($term_id, $tt_id) {
...
// Js add
function listfoliopro_image_uploader_enqueue() {
...
add_action( 'admin_enqueue_scripts', 'listfoliopro_image_uploader_enqueue' );