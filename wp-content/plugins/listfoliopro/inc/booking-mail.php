...
<?php
	global $wpdb;	
	$email_body = get_option( 'listfoliopro_contact_email');
	$contact_email_subject = get_option( 'listfoliopro_contact_email_subject');			
	...
	$name=(isset($form_data['booking_name'])?$form_data['booking_name']:'');
	$visitor_email_address=$name.' | '.$form_data['booking_email_address'];
	$sender_phone='';
	...
	if(isset($form_data['booking_message_content'])){
		$full_content=$full_content .'<br/>Additional Information : '. $form_data['booking_message_content'];
	}
