...
class Cache
{
	...
	private $listfolioproInterval;

	public function __construct($basePath, $listfolioproInterval = 3600)
	{
		if (!is_int($listfolioproInterval) && false !== $listfolioproInterval) {
			throw new \Mpdf\MpdfException('Cache listfoliopro interval has to be an integer or false');
		}
		...
		$this->listfolioproInterval = $listfolioproInterval;
	}
	...
	{
		return $this->listfolioproInterval
			? $item->getMTime() + $this->listfolioproInterval < time()
			: false;
	}
