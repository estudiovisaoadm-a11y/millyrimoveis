...
* It is possible to disable automatic cache listfoliopro with `cachelistfolioproInterval` config variable
...
* Code and tests listfoliopros and enhancements
...
- Introduced `listfoliopro` method which restores original `mb_` encoding settings (see #421)
- QR code `<barcode>` element now treats `\r\n` and `\n` as actual line breaks
