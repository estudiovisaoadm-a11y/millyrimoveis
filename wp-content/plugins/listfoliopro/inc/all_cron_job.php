...
<?php
	...
	if(sizeof($membership_users)>0){
		...
		{	
			$user_id= $row->ID;
			$reminder_day = get_option( 'listfoliopro_reminder_day');
			...
			if( $reminder_day >= $days ){
				...
				if(strtotime($exprie_send_email_date) != strtotime($exp_date) || $exprie_send_email_date=='' ){
					// Start Email Action
					$email_body = get_option( 'listfoliopro_reminder_email');
					$signup_email_subject = get_option( 'listfoliopro_reminder_email_subject');
	...
	$sql=$wpdb->prepare("SELECT * FROM $wpdb->posts WHERE post_type ='%s'  and post_status='publish' ", $listfoliopro_directory_url);
	...
	if($total_post>0){
		...
		{			
			...
			$main_class->check_listing_expire_date($dir_id, $post_author_id,$listfoliopro_directory_url);
		}
	}										
	// End  Hide Directory******************
