...
<?php
global $wpdb;			
	$email_body_cilent = get_option( 'listfoliopro_order_client_email');
	$order_email_subject_client = get_option( 'listfoliopro_order_client_email_sub');
					
		$admin_mail = get_option('admin_email');	
		if( get_option( 'listfoliopro_admin_email' )==FALSE ) {
			$admin_mail = get_option('admin_email');						 
		}else{
			$admin_mail = get_option('listfoliopro_admin_email');
		}						
	...
	$dis_amount =get_user_meta($userId, 'listfoliopro_discount',true);
	
	$package_amount_2=$listfoliopro_package_cost.' '.$api_currency;
	$discount_2= $dis_amount.' '.$api_currency;
	$total_g =((int)$listfoliopro_package_cost - (int)$dis_amount).' '.$api_currency;
	...
	// For Admin Email*****	
	$email_body_admin = get_option( 'listfoliopro_order_admin_email');
	$order_email_subject_admin = get_option( 'listfoliopro_order_admin_email_sub');
