<?php
	...
	// Fallback: load the latest listing when no detail slug is provided
	if (empty($listingid)) {
		$fallback_post = get_posts([
			'post_type' => $listinghub_directory_url,
			'posts_per_page' => 1,
			'post_status' => 'publish',
		]);
		if (!empty($fallback_post)) {
			$listingid = $fallback_post[0]->ID;
		} else {
			echo '<div class="bootstrap-wrapper"><div class="container"><div class="alert alert-warning">'.esc_html__('No listing found.','listfoliopro').'</div></div></div>';
			return;
		}
	}
	...
	<div class="container" >
		<div class="row box-white  p-4 mt-5 mb-4 m-2">
			...
			<?php
				...
				?>
				<div class="col-12 mt-4 text-left single-style-1-info-row">
