<?php
	...
	wp_enqueue_style('listfoliopro_single-listing', listfoliopro_ep_URLPATH . 'admin/files/css/single-listing-2.css');
	...
	if($field_set==""){
		$field_set= listfoliopro_default_custom_fields_with_type();
	}
...
<?php					   
	...
	<div class="list-single-slider">							
		<?php include(listfoliopro_ep_template . '/listing/single-template/single-style-2/slider.php'); ?>
	</div>
	<?php
	}
?>
	...
	<div class="container" >
		<div class="row box-white  p-4 mt-5 mb-4 m-2">
			...
			<?php
				...
				?>
				<div class="col-xl-6 col-md-12 mt-4 text-right">
					<h1>
						...
						<div class="single-listing-rating-count" >
							...
							<?php
								...
								if($avg_review!=$saved_listing_avg_rating){
									update_post_meta($listingid,'review',$avg_review);
								}
							?>
			...
			<ul class="horizontal-list  mt-3">
				<?php						
					if(get_post_meta( $listingid, 'bed', true )!=''){
						if ( array_key_exists( 'bed', $active_single_fields_saved ) ) {
						?>
						<li >
							<div class="mb-2">
								<?php 
							...
							<span class="mt-2 mb-2"><?php echo esc_html((isset($field_set['bed'])? $field_set['bed'].' . ':''));?></span>
							<?php echo esc_html(get_post_meta( $listingid, 'bed', true ));?>
						</li>
						<?php } 
					}
				?>
				<?php						
					if(get_post_meta( $listingid, 'bath', true )!=''){
						if ( array_key_exists( 'bath', $active_single_fields_saved ) ) {
						?>
						<li >
							<div class="mb-2">
								<?php 
							...
							<span class="mt-2 mb-2"><?php echo esc_html((isset($field_set['bath'])? $field_set['bath'].' . ':''));?></span>
							<?php echo esc_html(get_post_meta( $listingid, 'bath', true ));?>
						</li>
						<?php } 
					}
				?>
				<?php						
					if(get_post_meta( $listingid, 'kitchen', true )!=''){
						if ( array_key_exists( 'kitchen', $active_single_fields_saved ) ) {
						?>
						<li >
							<div class="mb-2">
								<?php 
									$saved_icon = listfoliopro_get_icon( $active_archive_icon_saved, 'kitchen', 'archive' );
									if($saved_icon!=''){
										echo '<img class="mr-1 field-icon-size" src="'.esc_url($saved_icon).'">';
									}?>
							</div>
							<span class="mt-2 mb-2"><?php echo esc_html((isset($field_set['kitchen'])? $field_set['kitchen'].' . ':''));?></span>
							<?php echo esc_html(get_post_meta( $listingid, 'kitchen', true ));?>
						</li>
						<?php } 
					}
				?>
				<?php						
					if(get_post_meta( $listingid, 'area', true )!=''){
						if ( array_key_exists( 'area', $active_single_fields_saved ) ) {
						?>
						<li >
							<div class="mb-2">
								<?php 
							...
							<span class="mt-2 mb-2"><?php echo esc_html(get_post_meta( $listingid, 'area_prefix_text', true )).' . '; ?></span>
							<?php echo esc_html(get_post_meta( $listingid, 'area', true ));?>
						</li>
						<?php } 
					}
				?>
				...
				<div class="single-listing-main-wrapper ">						
					...
					<div class=" row mt-4 mb-4">	
						...
						<?php
							...
							?>
							<button class="pl-4 pr-4" type="button" onclick="listfoliopro_listing_booking_popup('<?php echo esc_html( $listingid ); ?>')"><?php esc_html_e( 'Book a Site Visit', 'listfoliopro' ); ?></button>
							<?php
							}
					...
					<?php
						...
						<div class="box-white  p-4 mt-5 mb-4 ">
							...
							<?php include(listfoliopro_ep_template . '/listing/single-template/single-style-2/similar-listings-sharp.php'); ?>
