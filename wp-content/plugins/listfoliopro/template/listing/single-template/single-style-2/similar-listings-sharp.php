<?php
	...
	wp_enqueue_style('listfoliopro_listing_style_sharp', listfoliopro_ep_URLPATH . 'admin/files/css/archive-listing-grid-style-2-sharp.css');	
...
<div class="col-md-12 " >
    <div class="listfoliopro-item-s2 similar-block">
		<?php
		$listing_contact_source = get_post_meta( $similar_id, 'listing_contact_source', true );
		if ( $listing_contact_source == '' ) {
			$listing_contact_source = 'user_info';
		}
		if ( $listing_contact_source == 'new_value' ) {
			$company_name    = get_post_meta( $similar_id, 'company_name', true );
			$company_address = get_post_meta( $similar_id, 'address', true );
			$company_web     = get_post_meta( $similar_id, 'contact_web', true );
			$company_phone   = get_post_meta( $similar_id, 'phone', true );
			$company_email   = get_post_meta( $similar_id, 'contact-email', true );
		...
		if ( isset( $active_archive_fields['image'] ) ) {
            ...
            <div class="card-img-container">
                <a href="<?php echo get_the_permalink( $similar_id ); ?>">
                    <img src="<?php echo esc_html( $feature_img ); ?>" class="card-img-top-listing">
                </a>
				<?php
				if ( get_post_meta( $similar_id, 'property_status', true ) != '' ) {
					?>
                    <label class="btn-urgent-right"><?php echo esc_html(get_post_meta( $similar_id, 'property_status', true )); ?> </label>
					<?php
				}
				?>
				
				<?php
					if ( get_post_meta( $similar_id, 'listfoliopro_featured', true ) == 'featured' ) {
					?>
		...
		?>
				<div class="card-body">		   
					...
					<div class=" row location-date-wrapper mt-2 ">
						<?php
						if(get_post_meta( $similar_id, 'area', true )!=''){
						?>
						<div class="col-xl-4 col-md-12  location align-items-center pr-0">
							<?php 
							...
							<?php echo esc_html(get_post_meta( $similar_id, 'area', true )); ?><span class="ml-1"><?php echo esc_html(get_post_meta( $similar_id, 'area_prefix_text', true )); ?></span> 
						 </div>
						 <?php
						}
						 ?>
						 <?php
						if(get_post_meta( $similar_id, 'bed', true )!=''){
						?>
						 <div class="col-xl-4 col-md-12 location align-items-center pr-0">
							<?php 
							...
							<?php echo esc_html(get_post_meta( $similar_id, 'bed', true ));?><span class="ml-1"><?php echo esc_html((isset($field_set['bed'])? $field_set['bed']:''));?></span>
						 </div>
						 <?php
						}
						 ?>
						 <?php
						if(get_post_meta( $similar_id, 'bath', true )!=''){
						?>
						<div class="col-xl-4 col-md-12 location align-items-center pr-0">
							<?php 
							...
							<?php echo esc_html(get_post_meta( $similar_id, 'bath', true ));?><span class="ml-1"><?php echo esc_html((isset($field_set['bath'])? $field_set['bath']:''));?></span>
						 </div>
						<?php
						}
						?>
					...
					<?php
					if ( get_post_meta( $similar_id, 'price', true ) != '' or get_post_meta( $similar_id, 'discount', true ) != '' ) {
						?>
						<div class="listfoliopro-listing-price listing-desc">
							<span class="pull-left mt-4">
							<?php if ( get_post_meta( $similar_id, 'discount', true ) != '' ) { ?>
								<strike class="listfoliopro-main-price"><?php  echo esc_html( listfoliopro_get_price(get_post_meta( $similar_id, 'price', true ),$similar_id,$dir_number_format)); ?></strike>
								<span class="listfoliopro-discount-price"><?php echo esc_html( listfoliopro_get_price(get_post_meta( $similar_id, 'discount', true ),$similar_id,$dir_number_format)); ?></span>
								<?php
							} else { ?>
								<?php echo esc_html( listfoliopro_get_price(get_post_meta( $similar_id, 'price', true ),$similar_id, $dir_number_format));  ?>
								<?php
							}
							?>
							</span>
