<?php
	...
	wp_enqueue_style('listfoliopro_categories', listfoliopro_ep_URLPATH . 'admin/files/css/categories.css');
					...
					if($term_parent->count>0){
						...
						if($cate_main_image!=''){
							...
							if(get_option('listfoliopro_category_defaultimage')!=''){
								$feature_img= wp_get_attachment_image_src(get_option('listfoliopro_category_defaultimage'));
								if(isset($feature_img[0])){									
									$feature_img=$feature_img[0] ;
								}
								}else{
								$feature_img=listfoliopro_ep_URLPATH."/assets/images/category.png";
							}
						}
