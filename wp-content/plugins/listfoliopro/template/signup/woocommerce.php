...
<div class="row form-group">
	<label for="text" class="col-md-4"><?php  esc_html_e('Package Name','listfoliopro');?></label>
	<div class="col-md-8">
		<?php
			$recurring_text='';
			...
			if( $package_name==""){
				...
				if(sizeof($membership_pack)>0){
					...
					if($recurring == 'on'){
						$package_amount=get_post_meta($package_id, 'listfoliopro_package_recurring_cost_initial', true).' '.$api_currency.'/'.get_post_meta($package_id, 'listfoliopro_package_recurring_cycle_count', true).' '.get_post_meta($package_id, 'listfoliopro_package_recurring_cycle_type', true);
						}else{
						$package_amount=get_post_meta($package_id, 'listfoliopro_package_cost',true).' '.$api_currency;
					}
				?>
				<?php
				}
...
<div class="row form-group">
	<label for="text" class="col-md-4"><?php  esc_html_e('Amount','listfoliopro');?></label>
...
<div class="row">
	...
	<div class="col-md-8 ">
		...
		if($eprecaptcha_api==''){
			?>
				<button  id="submit_listfoliopro_payment" name="submit_listfoliopro_payment"  type="submit" class="btn btn-secondary"  > <?php  esc_html_e('Submit','listfoliopro');?></button>
		<?php
		}else{
		?>
