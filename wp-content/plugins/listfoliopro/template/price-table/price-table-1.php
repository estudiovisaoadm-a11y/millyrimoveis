<?php
	...
	wp_enqueue_style('listfoliopro_pricing-table', listfoliopro_ep_URLPATH . 'admin/files/css/pricing-table.css');
	...
	'posts_per_page'=> '200', 
	'orderby'       => 'meta_value',
	'meta_key'      => 'listfoliopro_display_order',
	'order'          => 'ASC',
	);
	...
	$color_setting=get_option('listfoliopro_color');
...
<div class="bootstrap-wrapper ">
	<div class="container ">
		...
		<?php
			...
			if($the_query->found_posts>0){
				$page_name_reg=get_option('listfoliopro_registration' );
				$feature_max=0;
				...
				{	$amount_only='';
					...
					if(get_post_meta($row->ID, 'listfoliopro_package_cost', true)=='0' or get_post_meta($row->ID, 'listfoliopro_package_cost', true)==""){
						...
						$amount= $currencyCode.' '. get_post_meta($row->ID, 'listfoliopro_package_cost', true);
						...
						<div class="pricing-head">
							...
							<p class="short-desc-price-table mb-3 mt-3"><?php echo esc_html(get_post_meta($row->ID,'package_subtitle',true)); ?> </p>
								...
								<div class="row mb-3 mt-3">
									<div class="col-12">
										<a class="btn btn-big mt-2" href="<?php echo get_page_link($page_name_reg).'?&package_id='.esc_attr($row->ID); ?>"><?php esc_html_e('BUY NOW','listfoliopro'); ?></a>
									</div>	
								</div>							
						</div>
					</div>
